...
fn eval_correction(td: &ThreadData, ply: isize) -> i32 {
    ...
    (corrhist.pawn.get(stm, td.board.pawn_key())
        + corrhist.kpn.get(stm, td.board.triplet_key(PieceType::King, PieceType::Pawn, PieceType::Knight)) / 2
        + corrhist.knr.get(stm, td.board.triplet_key(PieceType::King, PieceType::Knight, PieceType::Rook)) / 2
        + corrhist.kbr.get(stm, td.board.triplet_key(PieceType::King, PieceType::Bishop, PieceType::Rook)) / 2
        + corrhist.krq.get(stm, td.board.triplet_key(PieceType::King, PieceType::Rook, PieceType::Queen)) / 2
