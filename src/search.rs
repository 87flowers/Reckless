...
fn search<NODE: NodeType>(
    ...
    if best_move.is_present() {
        ...
        let quiet_bonus = (184 * depth).min(1742) - 72 - 42 * cut_node as i32 - 32 * (best_move == tt_move) as i32;
        let quiet_malus = (171 * depth).min(1099) - 46 - 31 * quiet_moves.len() as i32;

        let cont_bonus = (97 * depth).min(1098) - 74 - 48 * cut_node as i32 - 32 * (best_move == tt_move) as i32;
