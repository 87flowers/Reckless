...
pub struct HashHistory {
    // [hash][stm][from][to]
    entries: Box<[[FromToHistory<i16>; 2]; Self::SIZE]>,
}
...
impl HashHistory {
    ...
    pub fn get(&self, key: u64, stm: Color, mv: Move) -> i32 {
        self.entries[key as usize & Self::MASK][stm][mv.from()][mv.to()] as i32
    }

    pub fn update(&mut self, key: u64, stm: Color, mv: Move, bonus: i32) {
        let entry = &mut self.entries[key as usize & Self::MASK][stm][mv.from()][mv.to()];
        apply_bonus::<{ Self::MAX_HISTORY }>(entry, bonus);
    }
}
