...
pub fn slider_attacks_setwise(
    ...
    unsafe {
        let rb_attackers =
            _mm512_mask_blend_epi64(0x0F, _mm512_set1_epi64(rooks.0 as i64), _mm512_set1_epi64(bishops.0 as i64));
        let q_attackers = _mm512_set1_epi64(queens.0 as i64);

        let rot1 = _mm512_set_epi64(-8, -1, 1, 8, -9, -7, 7, 9);
        let rot2 = _mm512_add_epi64(rot1, rot1);
        let rot4 = _mm512_add_epi64(rot2, rot2);
        ...
        let q_prop = _mm512_and_si512(_mm512_set1_epi64(!occupancies.0 as i64), masks);
        let rb_gen = _mm512_or_si512(rb_gen, _mm512_and_si512(rb_prop, _mm512_rolv_epi64(rb_gen, rot1)));
        let q_gen = _mm512_or_si512(q_gen, _mm512_and_si512(q_prop, _mm512_rolv_epi64(q_gen, rot1)));
        let rb_prop = _mm512_and_si512(rb_prop, _mm512_rolv_epi64(rb_prop, rot1));
        let q_prop = _mm512_and_si512(q_prop, _mm512_rolv_epi64(q_prop, rot1));
        let rb_gen = _mm512_or_si512(rb_gen, _mm512_and_si512(rb_prop, _mm512_rolv_epi64(rb_gen, rot2)));
        let q_gen = _mm512_or_si512(q_gen, _mm512_and_si512(q_prop, _mm512_rolv_epi64(q_gen, rot2)));
        let rb_prop = _mm512_and_si512(rb_prop, _mm512_rolv_epi64(rb_prop, rot2));
        let q_prop = _mm512_and_si512(q_prop, _mm512_rolv_epi64(q_prop, rot2));
        let rb_gen = _mm512_or_si512(rb_gen, _mm512_and_si512(rb_prop, _mm512_rolv_epi64(rb_gen, rot4)));
        let q_gen = _mm512_or_si512(q_gen, _mm512_and_si512(q_prop, _mm512_rolv_epi64(q_gen, rot4)));
        let rb_attacks = _mm512_and_si512(_mm512_rolv_epi64(rb_gen, rot1), masks);
        let q_attacks = _mm512_and_si512(_mm512_rolv_epi64(q_gen, rot1), masks);

        // Fold attacks
        unsafe fn fold(x: __m512i) -> __m512i {
        ...
        let rb_attacks = fold(rb_attacks);
        let q_attacks = fold(q_attacks);

        let r_attacks = Bitboard(_mm512_test_epi8_mask(rb_attacks, _mm512_set1_epi8(0xF0u8 as i8)));
        let b_attacks = Bitboard(_mm512_test_epi8_mask(rb_attacks, _mm512_set1_epi8(0x0Fu8 as i8)));
        let q_attacks = Bitboard(_mm512_test_epi8_mask(q_attacks, q_attacks));

        (b_attacks, r_attacks, q_attacks)
    }
}
